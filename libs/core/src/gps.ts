...
/**
 * Stateful filter over a stream of fixes. Keeps only the last raw fix and the
 * smoothed output, so it is cheap to run inside a `watchPosition` callback.
 *
 * Plausibility is judged between consecutive *raw* fixes: the smoothed position
 * lags behind on coarse fixes, and measuring against it would report a speed
 * higher than the runner's and reject steady movement.
 */
export function createGpsFilter(options: Partial<GpsFilterOptions> = {}) {
  const config = { ...DEFAULT_GPS_FILTER_OPTIONS, ...options };
  let lastRaw: GpsFix | null = null;
  let smoothed: GpsFix | null = null;
  let distanceM = 0;
  ...
  return {
    ...
    push(fix: GpsFix): GpsFilterResult {
      ...
      const moved = distanceMeters(lastRaw, fix);
      const elapsedS = Math.max((fix.timestamp - lastRaw.timestamp) / 1000, 0.001);
      ...
      const next = smoothFix(smoothed, fix);
      distanceM += distanceMeters(smoothed, next);
      lastRaw = fix;
      smoothed = next;
      return { accepted: true, fix: next, distanceM: moved };
    },
  };
}
